...
class Balls(Problem):
    ...
    def __init__(self, initial, size, walls):
    ...
    def successor(self, state):
        ...
        for i, ball in enumerate(state):
            for a, d in actions.items():
                # Новата позиција
                x, y = ball[0] + d[0], ball[1] + d[1]
                # Средното (прескокнатото) поле
                middle = (ball[0] + d[2], ball[1] + d[3])

                # Проверка: целта е празна, а средното има топче
                if self.check_valid(state, x, y) and middle in state:
                    new_state = list(state)
                    new_state[i] = (x, y)       # помести го топчето
                    new_state.remove(middle)    # избриши го прескокнатото топче
    ...
    def goal_test(self, state):
        # Ова е истото што го имаше во оригиналниот код:
        return len(state) == 1 and state[0] == (self.size // 2, self.size - 1)

    def check_valid(self, state, x, y):
        return 0 <= x < self.size > y >= 0 and (x, y) not in self.walls and (x, y) not in state
if __name__ == "__main__":
    N = int(input())
    num_balls = int(input())
    balls = tuple([tuple(map(int,input().split(','))) for _ in range(num_balls)])

    num_walls = int(input())
    walls = tuple([tuple(map(int, input().split(','))) for _ in range(num_walls)])
