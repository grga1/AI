...
if __name__ == '__main__':
    ...
    for row in dataset:
        row[-1]=1 if row[-1]>=5 else 0

    split_at = int(len(dataset)*x/100)
    ...
    col = list(tree.feature_importances_).index(min(tree.feature_importances_))

    train_x = [row[:col]+row[(col+1):] for row in train_x]
    test_x = [row[:col]+row[(col+1):] for row in test_x]

    model = MLPClassifier(15,activation='relu',learning_rate_init=0.001,max_iter=200,random_state=0)
    ...
    print(f'Tocnost so StandardScaler: {model.score(test_x, test_y)}')

    min_max_scaler.fit(train_x)
    train_x = min_max_scaler.transform(train_x)
    test_x = min_max_scaler.transform(test_x)
    model.fit(train_x, train_y)
    print(f'Tocnost so MinMaxScaler: {model.score(test_x, test_y)}')
