...
if __name__ == '__main__':
    ...
    split_at = int(0.85 * len(dataset))
    train_data = dataset[:split_at]
    test_data = dataset[split_at:]

    x_Train = [[float(val) for val in row[:-1]] for row in train_data]
    y_Train = [int(row[-1]) for row in train_data]

    x_Testing = [[float(val) for val in row[:-1]] for row in test_data]
    y_Testing = [int(row[-1]) for row in test_data]

    classifier = GaussianNB()
    classifier.fit(x_Train, y_Train)

    accuracy = classifier.score(x_Testing, y_Testing)
    print(accuracy)
    query = [float(val) for val in input().split(" ")]
