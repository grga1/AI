...
if __name__ == '__main__':
    X = int(input())
    X=X/100
    zero = [row for row in dataset if row[-1]==0]
    one = [ row for row in dataset if row[-1]==1]

    train_data = one[:int(len(one)*X)] + zero[:int(len(zero)*X)]
    test_data = one[int(len(one)*X):] + zero[int(len(zero)*X):]
    ...
    forest = RandomForestClassifier(n_estimators=4,criterion='entropy',random_state=0)
    neural = MLPClassifier(hidden_layer_sizes=(10,),activation='relu',learning_rate_init=0.001,random_state=0)
    ...
    models = [bayes,tree,forest,neural]
    names = ['Naive Bayes','Decision Tree','Random Forest','MLP']
    accuracy = [bayes.score(test_x,test_y),tree.score(test_x,test_y),forest.score(test_x,test_y),neural.score(test_x,test_y)]
    predictions = [bayes.predict(test_x),tree.predict(test_x),forest.predict(test_x),neural.predict(test_x)]
    ...
    for x,y in zip(test_x,test_y):
        ...
        for i,model in enumerate(models):
            ...
            weight = 2 if i==best_index else 1
            votes[pred]+=weight
        final = 0 if votes[0]>votes[1] else 1
        if y == 1:
            if final==1: tp+=1
            else: fn+=1

    print(f'Najgolema tocnost ima klasifikatorot {names[best_index]}')
