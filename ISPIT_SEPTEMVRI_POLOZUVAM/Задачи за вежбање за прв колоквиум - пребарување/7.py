...
Со избор (кликнување) на едно поле се прави промена на бојата на тоа поле и на сите негови непосредни соседи (горе, долу, лево и десно) во спротивната боја, 
...
во променливата fields ја имате бојата на сите полиња на таблата (по редослед: одлево - надесно, редица по редица, ако таблата ја гледате како матрица), 
...
Вашиот код треба да има само еден повик на функција за приказ на стандарден излез (print) со кој ќе ја вратите секвенцата на потези која 
треба да се направи за да може сите полиња на таблата да бидат обоени во црна боја. 
...
class WishDotComGameOfLife(Problem):
    ...
    def successor(self, state):
        ...
        for x in range(self.size):
            for y in range(self.size):
                ...
                for space in nearby:
                    nx,ny = space[0]+x,space[1]+y
                    if not  0<=nx<self.size>ny>=0:
                        continue
                    
                    color = new_state[nx][ny]
                    if color == 0:
                        new_state[nx][ny] =1
                    else:
                        new_state[nx][ny]=0
...
if __name__ == "__main__":
    N = int(input())
    spaces = tuple(map(int,input().split(',')))

    matrix = []
    for i in range(N):
        row = spaces[i*N:(i+1)*N]
        matrix.append(row)

    matrix = tuple(matrix)
    problem = WishDotComGameOfLife(matrix,N)
