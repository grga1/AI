...
# Пример за почетна состојба е прикажан на следната слика:
...
# Ваша задача е да го имплементирате движењето на Духот во successor функцијата, така што најпрво ќе се проба акцијата за движење горе, а потоа десно. 
# Акциите се именуваат како „Gore/desno X“. Потоа имплементирајте ја хевристичката функција h. 
...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        pos = state
        for i in [1,2,3]:
            curr_pos = (pos[0],pos[1]+i)
            if self.check_valid(curr_pos,self.walls, self.n):
                successors[f"Gore {i}"] = curr_pos

        for i in [1,2,3]:
            curr_pos = pos[0]+i,pos[1]
            if self.check_valid(curr_pos,self.walls,self.n):
                successors[f"Desno {i}"] = curr_pos

        return successors

    def h(self, node):
        state = node.state
        return (abs(self.n-1-state[0])+abs(self.n-1-state[1]))/3
