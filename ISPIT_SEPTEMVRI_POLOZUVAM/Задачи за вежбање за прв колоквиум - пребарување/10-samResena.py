...
class Football(Problem):
    def __init__(self, initial,goal_pos,opponents):
    ...
    def goal_test(self, state):
        ...
        return  ball_pos == (7,2) or ball_pos == (7,3)
    ...
    def check_valid(self,state):
        ...
        if not ( 0<=x1<8 and 0<=y1<6):
            return False
        if not (0<=x2<8 and 0<=y2<6):
            return False
        if man_pos in self.opponents:
            return False
        if ball_pos in self.opponents:
            return False
        if man_pos == ball_pos:
            return False
        for dx,dy in surroundings:
            if (dx+ball_pos[0],dy+ball_pos[1]) in self.opponents:
                return False
        return True
    ...
    def successor(self, state):
        ...
        x1,y1 = man_pos
        x2,y2 = ball_pos
        moves = {'gore':(0,1),'dolu':(0,-1),'desno':(1,0),'gore-desno':(1,1),'dolu-desno':(1,-1)}
        ...
        for action,(dx,dy) in moves.items():
         ...
         if (nx1,ny1) == ball_pos:
             ...
             if self.check_valid(new_state):
              successors[f'Turni topka {action}']=new_state
         else:
             new_state = ((nx1, ny1), (x2,y2))
             if self.check_valid(new_state):
                 successors[f'Pomesti coveche {action}'] = new_state
...
if __name__ == '__main__':
    ...
    opponents = [(3, 3), (5, 4)]
    problem = Football(initial_state,goal_pos,opponents)
