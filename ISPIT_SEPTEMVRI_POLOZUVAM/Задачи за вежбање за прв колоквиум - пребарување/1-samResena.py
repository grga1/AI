...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        successors = dict()
        moves = {'Gore 1':(0,1),'Gore 2':(0,2),'Gore 3':(0,3),
                 'Desno 1': (1, 0), 'Desno 2': (2, 0), 'Desno 3': (3, 0)}
        ghost = state
        px,py = ghost
        for action,(dx,dy) in moves.items():
            npx,npy= px + dx, py + dy
            if self.check_valid((npx,npy),self.walls,self.n):
                successors[f'{action}']=(npx,npy)
        return successors

    def h(self, node):
        x,y = node.state
        gx,gy = self.goal
        return  abs((gx-x)+(gy-y))/3
