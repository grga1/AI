...
# Топката исто така не може да се наоѓа на поле кое е соседно со некој од противниците (хоризнотално, вертикално или дијагонално) или на исто поле со некој од противниците.
# На слика 1 е покажана една можна почетна состојба на таблата.
# За сите тест примери големината на таблата е иста, а позицијата на човечето и топката се менуваат и се читаат од стандарден влез.
# Позицијата на противниците и голот е иста за сите тест примери. 
...
# Дополнително, потребно е да проверите дали сте стигнале до целта, односно да ја имплементирате функцијата goal_test и
# да проверите дали состојбата е валидна, односно да ја дополните функцијата check_valid. 
...
class Football(Problem):
    ...
    def check_ball(self, state):
        ...
        for dx, dy in surroundings:
            if (ball[0] + dx, ball[1] + dy) in self.opponents:
                return False
    ...
    def successor(self, state):
        ...
        for a, d in actions.items():
            player = (state[0][0] + d[0], state[0][1] + d[1])
            if self.check_player((player, state[1])):
                if player == state[1]:  # ако играчот стигне на топката
                    ball = (state[1][0] + d[0], state[1][1] + d[1])
                    if self.check_ball((player, ball)):
                        successors[f'Turni topka {a}'] = (player, ball)
                else:
                    successors[f'Pomesti coveche {a}'] = (player, state[1])
        return successors
