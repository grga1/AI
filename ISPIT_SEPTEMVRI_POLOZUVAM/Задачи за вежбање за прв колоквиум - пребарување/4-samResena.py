...
class Squares(Problem):
    ...
    def successor(self, state):
        succ = dict()
        moves = {'gore':(0,1),'dolu':(0,-1),'levo':(-1,0),'desno':(1,0)}


        for index,square in enumerate(state):
            for action, (dx, dy) in moves.items():
                px,py =square[0]+dx,square[1]+dy
                new_state = list(state)
                new_state[index] = (px, py)

                if self.check_valid(tuple(new_state)):
                   succ[f'Pomesti kvadratche {index+1} {action}'] = tuple(new_state)
        return succ
