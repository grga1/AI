...
class Balls(Problem):
    ...
    def successor(self, state):
        successors = dict()
        moves = {'Gore Levo':(-2,2,-1,1),'Gore Desno':(2,2,1,1),'Dolu Levo':(-2,-2,-1,-1),'Dolu Desno':(2,-2,1,-1),'Levo':(-2,0,-1,0),'Desno':(2,0,1,0)}
        for action,d in moves.items():
         for index,ball in enumerate(state):
            nx,ny = ball[0]+d[0],ball[1]+d[1]
            if not self.check_valid(state,nx,ny):
                    continue
            new_state = list(state)
            odame_jako = (ball[0]+d[2],ball[1]+d[3])
            if odame_jako in state:
                new_state[index]=(nx,ny)
                new_state.remove(odame_jako)
                successors[f'{action}: (x={ball[0]},y={ball[1]})'] = tuple(new_state)
    ...
    def goal_test(self, state):
        return len(state)==1 and state[0] == ((int)(self.N/2),self.N-1)


if __name__ == "__main__":
    N = int(input())
    n_balls = int(input())
    balls = tuple(tuple(map(int, input().split(',')))for _ in range(n_balls))
    n_walls = int(input())
    walls = tuple(tuple(map(int, input().split(','))) for _ in range(n_walls))

    problem = Balls(balls,walls,N)
