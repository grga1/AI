...
# Со еден потег Духот може да се помести за една,
# две или три позиции горе или десно.
...
# Почетната позиција на Духот секогаш е (0, 0),
# додека позицијата на Пакман секогаш е (n-1, n-1).
...
class GhostOnSkates(Problem):
    ...
    def check_valid(self,state):
        ...
        return 0<=x<self.n and 0<=y<self.n and state not in self.walls


    def successor(self, state):
        successors = dict()

        akcii = {'Gore':(0,1),'Desno':(1,0)}
        

        for eden,dva in akcii.items():
            for i in range (1,4):
               x,y = state
               dx,dy = dva
               nov_state = (x+dx*i,y+dy*i);
               
               if self.check_valid(nov_state):
                   successors[f'{eden} {i}']=nov_state
    ...
    def h(self, node):
        ...
        return (abs(x-goalx)+abs(y-goaly))/3
